...
- **Arbitrary→named overlap** (`enforce-canonical` ↔ `no-unnecessary-arbitrary-value` ↔ `prefer-theme-tokens`): three rules can transform an arbitrary value into a named utility, each owning a distinct case so they don't double-fire on a given input.
  ...
  - `prefer-theme-tokens` (off by default) is a name-match heuristic: for `prefix-(--name)` / `prefix-[var(--name)]`, if `prefix-name` is a valid utility it suggests the replacement. It guards against double-reports by skipping cases where `cache.getNamedEquivalent(bareUtility)` already maps to the same target. Useful in shadcn-style themes where `--X` is a raw variable but `--color-X` is the actual theme token (CSS is NOT equivalent — replacement may change observable output, hence opt-in).
  - Convergence: `border-[var(--border)]` reaches `border-border` via either `enforce-canonical → prefer-theme-tokens` (two iterations) or `prefer-theme-tokens` alone (one). The coexistence matrix is locked down in `tests/integration/prefer-theme-tokens-coexistence.test.ts`.
- **Known limitation in `arbitraryEquivalents`**: the precompute step in `sync-loader.ts` derives the candidate prefix via `cls.lastIndexOf('-')`, which truncates incorrectly for multi-segment utilities like `bg-red-500` (yields prefix `bg-red`, producing `bg-red-[var(--color-red-500)]` instead of `bg-[var(--color-red-500)]`). Single-segment utilities (`rounded-sm`, `h-auto`, `m-auto`) map correctly. Affected utilities still get canonicalized via `enforce-canonical` (uses Tailwind's API directly), so users see the fix, but `getNamedEquivalent` returns null for the bracket form. Not a blocker for `prefer-theme-tokens` (it has its own logic) but worth fixing for `no-unnecessary-arbitrary-value` coverage.

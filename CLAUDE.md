...
- **Sort service lifecycle**: `sort-service.ts` spawns a Worker thread on first `enforce-sort-order` use. Uses `SharedArrayBuffer` + `Atomics.wait()` with timeouts (30s init, 10s per request). Tracks `currentCssPath` and restarts the worker when the entry point changes (monorepo support). The worker uses `worker.unref()` so the process can exit without waiting — no `process.on('exit')` listener is registered (that would trip `MaxListenersExceededWarning` when the plugin runs in many oxlint worker threads). If the worker fails or times out, default-mode sorting is skipped with a one-time warning because heuristic sorting cannot safely mirror all official Tailwind order-dependent cases. `cache.getClassOrder()` remains used by strict mode and internal tests.
...
- **Arbitrary→named overlap** (`enforce-canonical` ↔ `no-unnecessary-arbitrary-value` ↔ `prefer-theme-tokens`): three rules can transform an arbitrary value into a named utility, each owning a distinct case so they don't double-fire on a given input.
  ...
  - `prefer-theme-tokens` (off by default) is a name-match heuristic: for `prefix-(--name)`, if `prefix-name` is a valid utility it suggests the replacement. It intentionally ignores bracket `var()` forms (`prefix-[var(--name)]`), which first canonicalize to paren shorthand via `enforce-canonical` or are handled by `no-unnecessary-arbitrary-value` when exact CSS equivalence is proven. Useful in themes where `--X` is a raw variable but `--color-X` wraps it in a color function (e.g. `hsl(var(--X))`) — CSS is NOT equivalent, replacement may change observable output, hence opt-in.
  - Convergence: when the theme exposes the variable directly (`--color-border: var(--border)`), `border-[var(--border)]` is CSS-equivalent to `border-border` and is owned by `no-unnecessary-arbitrary-value`; `border-(--border)` is owned by `prefer-theme-tokens`. When the theme wraps the variable in a color function (`--color-border: hsl(var(--border))`) the bracket form canonicalizes to `border-(--border)` first, then `prefer-theme-tokens` can suggest `border-border` if enabled. The coexistence matrix is locked down in `tests/integration/prefer-theme-tokens-coexistence.test.ts`.

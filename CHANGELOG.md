...
- **Fix Sonner/shadcn false positives** — Sonner marker classes (`toast`, `toaster`) are now treated as known non-Tailwind classes so shadcn's `<Sonner className="toaster group" />` wrapper does not fail `no-unknown-classes`.
- 1108 tests (up from 1103).
